...
import com.tttsaurus.fluxloading.render.shader.uniform.Variant;
import com.tttsaurus.fluxloading.util.TypeUtils;
...
public class ShaderProgram implements Comparable<ShaderProgram>, IGlDisposable {
    ...
    public String getSetupDebugReport() {
        ...
        for (Shader shader : shaders) {
            builder.append(
                ...
                .append("\n")
                .append("    - Validity: ")
                .append(shader.getValidity() ? "valid" : "invalid")
                .append("\n");
            if (!shader.getValidity()) builder.append("    - Error Log: ")
                .append(shader.getErrorLog())
                .append("\n");
        }
        ...
        for (int shaderID : shaderIDs) {
            ...
            if (shader != null) {
                builder.append(
                    ...
                    .append("\n")
                    .append("    - Shader GL ID: ")
                    .append(shader.getShaderID())
                    .append("\n")
                    .append("    - Uniform Fields:")
                    .append(
                        (shader.getUniformFields()
                            .isEmpty() ? " (Empty)" : ""))
                    .append("\n");
                for (UniformField field : shader.getUniformFields())
                    builder.append(String.format("      - [%s] %s", field.getType(), field.getFieldName()))
                        .append("\n");
            }
        }

        builder.append("\nShader Program:\n")
            .append("  - Program GL ID: ")
            .append(programID)
            .append("\n")
            .append("  - Program Uniform Fields:")
            .append((uniformFields.isEmpty() ? " (Empty)" : ""))
            .append("\n");
        for (Map.Entry<UniformField, Integer> entry : uniformFields.entrySet()) builder
            .append(
                String.format(
                    "    - [%s] %s, GL Loc: %d",
                    entry.getKey()
    ...
    private Shader getShaderByID(int shaderID) {
        for (Shader shader : shaders) if (shaderID == shader.getShaderID()) return shader;
        return null;
    }
    ...
    public void setup() {
        ...
        for (Shader shader : shaders) {
            shader.compile();
            if (shader.getValidity()) shaderIDs.add(shader.getShaderID());
        }
        for (int shaderID : shaderIDs) {
            Shader shader = getShaderByID(shaderID);
            if (shader != null) {
        ...
        for (int shaderID : shaderIDs) GL20.glAttachShader(programID, shaderID);

        GL20.glLinkProgram(programID);

        Iterator<Map.Entry<UniformField, Integer>> iterator = uniformFields.entrySet()
            .iterator();
        while (iterator.hasNext()) {
            Map.Entry<UniformField, Integer> entry = iterator.next();
            int uniformLocation = GL20.glGetUniformLocation(
                programID,
                entry.getKey()
                    .getFieldName());

            if (uniformLocation != -1) entry.setValue(uniformLocation);
    ...
    private float getFloat(Class<?> clazz, Object value) {
        ...
        else if (TypeUtils.isIntOrWrappedInt(clazz)) v = (float) ((int) value);
        return v;
    }

    private static double getDouble(Class<?> clazz, Object value) {
        double v = 0;
        if (TypeUtils.isDoubleOrWrappedDouble(clazz)) v = (double) value;
        else if (TypeUtils.isIntOrWrappedInt(clazz)) v = (double) ((int) value);
        else if (TypeUtils.isFloatOrWrappedFloat(clazz)) v = (double) ((float) value);
        return v;
    }

    private int getInt(Class<?> clazz, Object value) {
        int v = 0;
        if (TypeUtils.isIntOrWrappedInt(clazz)) v = (int) value;
        return v;
    }
    ...
    private int getUint(Class<?> clazz, Object value) {
        int v = 0;
        if (TypeUtils.isIntOrWrappedInt(clazz)) v = (int) value;
        else if (TypeUtils.isLongOrWrappedLong(clazz)) v = (int) (((long) value) & 0xFFFFFFFFL);
        return v;
    }

    private int getBool(Class<?> clazz, Object value) {
        int v = 0;
        if (TypeUtils.isBooleanOrWrappedBoolean(clazz)) v = ((boolean) value) ? 1 : 0;
        else if (TypeUtils.isIntOrWrappedInt(clazz)) v = ((int) value) > 0 ? 1 : 0;
        return v;
    }
