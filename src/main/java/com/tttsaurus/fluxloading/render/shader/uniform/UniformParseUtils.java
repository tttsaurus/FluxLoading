...
public final class UniformParseUtils {
    ...
    public static Tuple<UniformType, String> getUniformType(String rawType, String fieldName) {
        ...
        if (isArray(fieldName)) {
            ...
            for (; i >= 0; i--) if (fieldName.charAt(i) == '[') break;
            if (i > 0) {
                ...
                try {
                    ...
                    type = UniformType.ARRAY.deepClone()
        ...
        for (UniformType uType : UniformType.TYPE_PRESET) {
            ...
            if (rawType.equals(symbol)) {
                if (array) type.setSubType(cloned);
                else type = cloned;
                type.setSymbol(symbol);
            } else if (rawType.equals("i" + symbol)) {
                if (array) type.setSubType(cloned.setVariant(Variant.I))
                    .setVariant(Variant.I);
                else type = cloned.setVariant(Variant.I);
                type.setSymbol(symbol);
            } else if (rawType.equals("u" + symbol)) {
                if (array) type.setSubType(cloned.setVariant(Variant.U))
                    .setVariant(Variant.U);
                else type = cloned.setVariant(Variant.U);
                type.setSymbol(symbol);
            }
        }
